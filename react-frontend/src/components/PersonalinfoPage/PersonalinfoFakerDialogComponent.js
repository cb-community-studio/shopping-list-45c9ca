...
import personalinfoFakerFactory from "./personalinfoFakerFactory";
...
const PersonalinfoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = personalinfoFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("personalinfo").create(elem));
...
export default connect(null, mapDispatch)(PersonalinfoFakerDialogComponent);
