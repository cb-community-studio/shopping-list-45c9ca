...
import personal-infoFakerFactory from "./personal-infoFakerFactory";
...
const Personal-infoFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = personal-infoFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("personal-info").create(elem));
...
export default connect(null, mapDispatch)(Personal-infoFakerDialogComponent);
