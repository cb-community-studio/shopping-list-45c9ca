...
import shoppinglistFakerFactory from "./shoppinglistFakerFactory";
...
const ShoppinglistFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = shoppinglistFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("shoppinglist").create(elem));
...
export default connect(null, mapDispatch)(ShoppinglistFakerDialogComponent);
